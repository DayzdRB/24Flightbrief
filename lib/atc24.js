...
const FLIGHT_PLANS_KEY = '24data:flightplans:websocket';
// 24data only streams flight plans over its WebSocket, so filed callsigns are
// available whenever the relay worker is running. Kept well under the 14-day
// retention limit in the 24data terms.
const FLIGHT_PLAN_RETENTION_MS = 6 * 60 * 60 * 1000;
...
// Recent filed flight plans captured by the WebSocket relay. There is no REST
// fallback because 24data does not expose flight plans over REST. Returns an
// empty list when the relay is offline so callers can degrade gracefully.
async function getFlightPlans() {
  const snapshot = parseSnapshot(await redisGet(FLIGHT_PLANS_KEY).catch(() => null));
  if (!snapshot || !Array.isArray(snapshot.data)) return { fetchedAt: 0, data: [], source: 'none' };
  const cutoff = Date.now() - FLIGHT_PLAN_RETENTION_MS;
  return {
    ...snapshot,
    source: 'websocket',
    data: snapshot.data.filter(plan => Number(plan?.filedAt) >= cutoff),
  };
}
