...
function routeAirspaceAnalysis(data) {
  ...
  let amendmentLimitNode = null;
  if (firstTransition) {
    for (let index = firstTransition.legIndex + 1; index < nodes.length; index += 1) {
      if (nodes[index].sector?.id === firstTransition.toSector?.id) {
        amendmentLimitNode = nodes[index];
        break;
      }
    }
    if (!amendmentLimitNode) amendmentLimitNode = nodes[Math.min(nodes.length - 1, firstTransition.legIndex + 1)] || null;
  }
  ...
  const waypointCount = nodes.filter(node => node.kind === 'waypoint').length;
  const amendmentLimitWaypointIndex = amendmentLimitNode?.kind === 'waypoint'
    ? amendmentLimitNode.waypointIndex
    : waypointCount;
  ...
  return {
    ...
    startingSector,
    firstTransition,
    nextSector: firstTransition?.toSector || null,
    amendmentLimitNode,
    amendmentLimitWaypointIndex,
...
function validateAmendedWaypoints(filedData, amendedWaypoints) {
  ...
  const lockIndex = Math.min(filed.length, Math.max(0, analysis.amendmentLimitWaypointIndex));
  ...
  if (lockedSuffix.length) {
    ...
    if (amendedSuffix.length !== lockedSuffix.length || amendedSuffix.some((code, index) => code !== lockedSuffix[index])) {
      return {
        valid: false,
        error: `The filed route from ${lockedSuffix[0]} onward belongs to the next airspace and must remain unchanged.`,
        analysis,
      };
    }
  }
  ...
  return { valid: true, analysis, lockIndex, lockedSuffix };
}
