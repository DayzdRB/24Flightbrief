...
async function ensurePlanShape(plan, ownerId) {
  ...
  const normalizedNavigation = { ...defaultNavigation(), ...(plan.navigation || {}) };
  if (!plan.navigation || Object.keys(normalizedNavigation).some(key => plan.navigation[key] === undefined)) {
    changed = true;
  }
  plan.navigation = normalizedNavigation;
